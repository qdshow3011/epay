...
if(isset($_GET['kw'])) {
	...
	$numrows=$DB->count('invitecode', ['code'=>$kw]);
	$con='包含 '.$kw.' 的共有 <b>'.$numrows.'</b> 个邀请码';
}else{
	$numrows=$DB->count('invitecode', []);
...
if(isset($_GET['kw'])) {
	$rs=$DB->findAll('invitecode', '*', ['code'=>$kw], 'id desc', [$offset, $pagesize]);
} else {
	$rs=$DB->findAll('invitecode', '*', [], 'id desc', [$offset, $pagesize]);
}
foreach($rs as $res)
{
